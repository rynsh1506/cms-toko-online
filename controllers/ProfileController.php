        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                if ($action === 'update_profile') {
                    ...
                    $_SESSION['name'] = sanitize_input($_POST['name'] ?? $_SESSION['name']);

                    $this->sendResponse($is_ajax, true, 'Profil berhasil diperbarui.', 'index.php?page=admin_profile');
                } elseif ($action === 'change_password') {
                    $profileService->changePassword($_SESSION['user_id'], $_POST);

                    $this->sendResponse($is_ajax, true, 'Password berhasil diubah.', 'index.php?page=admin_profile');
                }
            } catch (\Exception $e) {
                $this->sendResponse($is_ajax, false, $e->getMessage(), 'index.php?page=admin_profile');
            }
        } else {
