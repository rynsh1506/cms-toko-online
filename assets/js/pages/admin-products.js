...
function fetchProducts(page = 1) {
  ...
  fetch(
    `index.php?page=admin_product_process&action=fetch&${searchParams.toString()}`,
  )
...
function renderTable(products) {
  ...
  products.forEach((product) => {
    ...
    const displayStock = hasVars
      ? totalVariantStock + normalStock
      : normalStock;
    const stockLabel = hasVars
      ? ` pcs (Total: ${normalStock} Normal + ${totalVariantStock} Varian)`
      : " pcs";
  ...
  $(document).on("click", ".btn-delete-product", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url:
            "index.php?page=admin_product_process&action=delete&id=" +
            id +
            "&ajax=1",
          type: "GET",
          dataType: "json",
...
function openVariantModal(productId, productName) {
  ...
  document.getElementById("variant-modal-product-name").textContent =
    productName;
...
function loadVariants() {
  ...
  $.getJSON(
    "index.php?page=admin_variant_process&action=list&product_id=" +
      currentProductId,
    function (data) {
      if (data.status !== "success") {
        list.innerHTML =
          '<p class="text-xs text-rose-500 text-center py-4">Gagal memuat varian.</p>';
        return;
      }
      ...
      // Group header
      const header =
        '<div class="grid grid-cols-12 gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider px-3 mb-1"><span class="col-span-3">Nama</span><span class="col-span-3">Nilai</span><span class="col-span-3">+Harga</span><span class="col-span-2">Stok</span><span class="col-span-1"></span></div>';
        ...
        list.innerHTML += `
            <div class="variant-row grid grid-cols-12 gap-2 items-center bg-slate-50 dark:bg-slate-700/50 rounded-xl px-3 py-2 text-sm" data-id="${v.id}">
                ...
                <div class="col-span-1 flex space-x-1">
                    <button onclick="openEditVariantModal(${v.id}, '${escHtml(v.variant_name)}', '${escHtml(v.variant_value)}', ${v.additional_price}, ${v.stock})" class="p-1 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/40 text-slate-400 hover:text-indigo-600 transition" title="Edit">
...
$("#add-variant-form").on("submit", function (e) {
  ...
  $.post(
    "index.php?page=admin_variant_process&action=add",
    {
...
// Edit Variant (Menggunakan Modal Form dengan SweetAlert)
function openEditVariantModal(id, name, value, price, stock) {
  document.getElementById("edit-variant-id").value = id;
  document.getElementById("edit-variant-name").value = name;
  document.getElementById("edit-variant-value").value = value;
...
function closeEditVariantModal() {
  ...
  document.getElementById("edit-variant-form").reset();
}

// Handle Edit Variant Form Submission
document
  .getElementById("edit-variant-form")
  ?.addEventListener("submit", function (e) {
    e.preventDefault();

    const id = document.getElementById("edit-variant-id").value;
    const name = document.getElementById("edit-variant-name").value.trim();
    const value = document.getElementById("edit-variant-value").value.trim();
    const price = document.getElementById("edit-variant-price").value;
    const stock = document.getElementById("edit-variant-stock").value;

    if (!name || !value) {
      Swal.fire({
        icon: "warning",
        title: "Data Tidak Lengkap",
        text: "Nama dan Nilai Varian harus diisi.",
      });
      return;
    }

    const btnSubmit = document.getElementById("btn-submit-edit-variant");
    const originalBtnText = btnSubmit.innerHTML;
    btnSubmit.disabled = true;
    btnSubmit.textContent = "Menyimpan...";

    $.post(
      "index.php?page=admin_variant_process&action=edit",
      {
        id: id,
        variant_name: name,
        variant_value: value,
...
// Close Edit Variant Modal on backdrop click
document
  .getElementById("edit-variant-modal")
  ?.addEventListener("click", function (e) {
    if (e.target === this) closeEditVariantModal();
  });
...
function deleteVariant(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post(
        "index.php?page=admin_variant_process&action=delete",
        { id: id },
        function (data) {
          if (data.status === "success") {
            loadVariants();
