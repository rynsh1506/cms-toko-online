$(document).ready(function() {
    // OTP Digits Navigation Logic (copied from verify.js)
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    `);
                    btn.prop('disabled', false).text('Verifikasi Kode');
                }
            },
            error: function() {
                $('#alert-container').html(`
    ...
    // Resend Reset Code with Countdown
    let countdown = 0;
    let timerInterval;
    ...
    $('#btn-resend').on('click', function() {
        ...
        if (!email) {
            $('#alert-container').html(`
                <div class="bg-rose-50 dark:bg-rose-950/20 border-l-4 border-rose-500 text-rose-800 dark:text-rose-400 p-3.5 rounded-r-xl mb-4 text-xs font-semibold">
                    Harap isi alamat email terlebih dahulu!
                </div>
            `);
            return;
        }
        ...
        $.ajax({
            url: 'index.php?page=forgot_password_process&action=resend_code',
            type: 'POST',
            data: { email: email },
            dataType: 'json',
            success: function(response) {
